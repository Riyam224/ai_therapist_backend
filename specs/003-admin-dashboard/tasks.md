...
- [ ] T022 [P] [US2] Test in `accounts/tests.py`: mock `accounts.views.firebase_auth_admin.delete_user` (or the equivalent import used by `accounts/admin.py`) to succeed, create a test user with `MoodEntry` rows, invoke the admin action (via Django admin test client `POST` with the action name and confirmation flag, or by calling the action function directly with a mocked `request`), then assert directly against the database that the `User` row and all matching `MoodEntry` rows no longer exist — same "assert on real row counts, not just mock calls" pattern as the existing deletion tests referenced in `README.md`
- [ ] T023 [P] [US2] Test in `accounts/tests.py`: mock the Firebase delete call to raise, invoke the action, assert the `User` row and `MoodEntry` rows still exist (fail-closed, FR-009) and that the admin shows an error message rather than a success message

### Implementation for User Story 2

- [ ] T024 [US2] In `accounts/admin.py`, import `delete_user_account` from `accounts.services` and add an `@admin.action(description="Delete account and journal entries")` method on `UserAdmin` that, for each selected user, calls `delete_user_account(user)` inside a `try`/`except`, using `self.message_user(request, ..., level=messages.ERROR)` on failure (mirroring `DeleteAccountView`'s fail-closed handling) and a success message otherwise
- [ ] T025 [US2] Implement the required confirmation step for the action in `accounts/admin.py` (check for `request.POST.get("post")` or an equivalent confirmation flag; if absent, render an intermediate confirmation `TemplateResponse` instead of deleting — per research.md §5's documented Django pattern) backed by a new template `templates/admin/accounts/user/delete_account_confirmation.html` listing the selected user(s) and warning the action is irreversible
- [ ] T026 [US2] Register the action on `UserAdmin.actions` in `accounts/admin.py`
