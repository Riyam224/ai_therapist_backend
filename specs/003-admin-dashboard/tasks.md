...
- [X] T039 Deployed to Railway; live `/admin/login/` confirmed 200 with Jazzmin theme assets and no traceback (verified via curl), and the requester confirmed by hand that the "Delete account and journal entries" bulk action shows and works correctly on the live deploy for `UserAdmin` — SC-006 gate satisfied
